...
export default function EditarEtapa(){
  ...
  const breadcrumbs = [
    ...
    <Link underline="hover" key="2" color="inherit" component={NavLink} to='/Etapas/Listar'>
      Etapas
    </Link>,
  ...
  const handleSubmit = (e) =>{
    ...
    EtapaService.actualizar({id,nombre,usuario:currentUser.id})
        .then(res => {
          dispatch(detailMessage({detailMessage:res.message,color:'success',showMessage:true}))
          navigateTo('/Etapas/Listar')
        })
        .catch(error => {
