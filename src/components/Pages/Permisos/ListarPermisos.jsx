...
export default function ListarPermisos(props){
  ...
  const columns = [
      { id: 'id', name: 'Id' },
      { id: 'titulo', name: 'Nombre' },
      { id: 'ruta', name: 'Ruta' },
      { id: 'visible', name: 'Visible' },
      { id: 'orden', name: 'Orden de despliegue' },
  ...
  const functionadd = () => {
    navigateTo('/Permisos/Crear')
  }

  const handleEdit = (code) => {
    navigateTo('/Permisos/Editar/'+code)
  }
  ...
  const showSwal = (code) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
          PermisoService.eliminarPermiso({id:code,usuario:currentUser.id})
            .then(res => {
                              ...
                              .map((row, i) => {
                                  return (
                                      <TableRow key={i}>
                                          ...
                                          <TableCell>{row.visible == 1 ? 'Si' : 'No'}</TableCell>
